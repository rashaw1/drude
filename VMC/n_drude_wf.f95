module n_drude_wf
  ...
  real(dbl),  dimension(:),    allocatable :: alpha, mu, q ! Parameters
...
contains
  ...
  subroutine init_drude(p_N, p_alpha, p_mu, p_q, p_gamma, p_eta, p_epsilon)
    ...
    real(dbl), dimension(p_N), intent(in) :: p_alpha, p_mu, p_q
  ...
  subroutine calc_wf_derivatives(p_rk, p_R, wf, grads, laplacian)
    ...
    real(dbl) :: Tijk, laplacian_temp

    ! Unpack p_rk, p_R into more useful form
    do i = 1, N-1
       rk(i, :) = p_rk(i, :) - p_R(i, :)
       R(i, :) = p_R(i+1, :) - p_R(i, :)
    enddo
    ...
    outer2: do i = 1, N-1
       inner2: do j = i+1, N
          wf = wf + ( mu(i) * mu(j) * q(i) * q(j) )/(mu(i) + mu(j)) * Jfunc(cvals(i, j), gamma(i, j))
          wf = wf - mu(i) * q(i) * q(j) * Jfunc(avals(i, j), eta(i, j))
          wf = wf - mu(j) * q(i) * q(j) * Jfunc(bvals(j, i), epsilon(j, i))
       end do inner2
    end do outer2
    
    do i = 1, N
       wf = wf - alpha(i)*dot_product(rk(i, :), rk(i, :))
    end do
    ! Exponentiate
    wf = exp(wf)
    ...
    kloop: do k = 1, N
       ...
       outer3: do i = 1, N-1
          inner3: do j = i+1, N
             ...
             grads(k, :) = grads(k, :) + (mu(i) * mu(j) * q(i) * q(j) )/(mu(i) + mu(j)) * Tijk*tau_c(i, j)*cvecs(i, j, :)
             grads(k, :) = grads(k, :) + mu(i) * q(i) * q(j) * delta(i, k)*tau_a(i, j)*avecs(i, j, :)
             grads(k, :) = grads(k, :) + mu(j) * q(i) * q(j) * delta(j, k)*tau_b(j, i)*bvecs(j, i, :)
          end do inner3
       end do outer3

       grads(k, :) = grads(k, :) - 2.0d0*alpha(k)*rk(k, :)
    end do kloop
    ...
    kloop2: do k = 1, N
       outer4: do i = 1, N-1
          inner4: do j = i+1, N
             ...
             laplacian_temp = (mu(i) * mu(j) * q(i) * q(j))/(mu(i) + mu(j)) * Tijk*xi_c(i, j)
             laplacian_temp = laplacian_temp - mu(i) * q(i) * q(j) * delta(i, k)*xi_a(i, j)
             laplacian_temp = laplacian_temp - mu(j) * q(i) * q(j) * delta(j, k)*xi_b(j, i)
          end do inner4
       end do outer4

       laplacian_temp = laplacian_temp - 6d0 * alpha(k) + dot_product(grads(k, :), grads(k, :))
       laplacian = laplacian + laplacian_temp/mu(k)
    end do kloop2
